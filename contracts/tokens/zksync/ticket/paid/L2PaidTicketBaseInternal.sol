...
abstract contract L2PaidTicketBaseInternal is IL2PaidTicketBaseInternal, AccessControlInternal, ERC721BaseInternal, ERC721EnumerableInternal {
    ...
    function _refundAndBurnTickets(uint256 _naffleId, uint256[] memory _naffleTicketIds, address _owner) internal {
        ...
        (bool success, ) = owner.call{value: length * naffle.ticketPriceInWei}("");
        if (!success) {
            revert RefundFailed();
        }

        emit PaidTicketsRefundedAndBurned(owner, _naffleId, totalTicketIds, _naffleTicketIds);
    }
